...
class MenuHelper():
    ...
    def choose_character(
        ...
        if controlling_port not in gamestate.players:
            controller.release_all()
            #print("Controller port not in gamestate")
            return
        ...
        if correct_character and coin_down:
            #print(f"Coin down: {target_character}, {target_x}, {target_y}, start: {start}") 
            #print(f"gamestate.ready_to_start: {gamestate.ready_to_start}")
            
            #print(f"gamestate.frame: {gamestate.frame}")
            if gamestate.frame % 2 == 0:
               controller.release_all()
               # print(f"Releasing all: {target_character}, {target_x}, {target_y}")
               return
        ...
        if isOverCharacter:
            #print(f"Over character: {target_character}, {target_x}, {target_y}")
            ...
            #Press A to select our character
                ...
                if controller.prev.button[enums.Button.BUTTON_A] == False:
                    controller.press_button(enums.Button.BUTTON_A)
                    #print(f"Selecting character: {target_character}")
                    return
                else:
                    controller.release_button(enums.Button.BUTTON_A)
                    return
            ...
            #Move in
            #print("Try to move in")
            controller.release_button(enums.Button.BUTTON_A)
            #Move up if we're too low
            if cursor_y < target_y - wiggleroom:
                controller.tilt_analog(enums.Button.BUTTON_MAIN, .5, 0.75)
                #print(f"x:{cursor_x} y:{cursor_y} target_x:{target_x} target_y:{target_y}")
                return
            #Move down if we're too high
            if cursor_y > target_y + wiggleroom:
                controller.tilt_analog(enums.Button.BUTTON_MAIN, .5, 0.25)
                #print(f"x:{cursor_x} y:{cursor_y} target_x:{target_x} target_y:{target_y}")
                return
            #Move right if we're too left
            if cursor_x < target_x - wiggleroom:
                controller.tilt_analog(enums.Button.BUTTON_MAIN, 0.75, .5)
                #print(f"x:{cursor_x} y:{cursor_y} target_x:{target_x} target_y:{target_y}")
                return
            #Move left if we're too right
            if cursor_x > target_x + wiggleroom:
                controller.tilt_analog(enums.Button.BUTTON_MAIN, 0.25, .5)
                #print(f"x:{cursor_x} y:{cursor_y} target_x:{target_x} target_y:{target_y}")
                return
        controller.release_all()
    ...
    def choose_stage(
        ...
        if gamestate.frame < 20:
            #print("Waiting for stage select to be ready")
            controller.release_all()
            return
        ...
        if gamestate.players[controller.port].cursor.y < target_y - wiggleroom:
            controller.release_button(enums.Button.BUTTON_A)
            controller.tilt_analog(enums.Button.BUTTON_MAIN, .5, .75)
            #print(f"x:{gamestate.players[controller.port].cursor.x} y:{gamestate.players[controller.port].cursor.y} target_x:{target_x} target_y:{target_y}")
            return
        #Move downn if we're too high
        if gamestate.players[controller.port].cursor.y > target_y + wiggleroom:
            controller.release_button(enums.Button.BUTTON_A)
            controller.tilt_analog(enums.Button.BUTTON_MAIN, .5, .25)
            #print(f"x:{gamestate.players[controller.port].cursor.x} y:{gamestate.players[controller.port].cursor.y} target_x:{target_x} target_y:{target_y}")
            return
        #Move right if we're too left
        if gamestate.players[controller.port].cursor.x < target_x - wiggleroom:
            controller.release_button(enums.Button.BUTTON_A)
            controller.tilt_analog(enums.Button.BUTTON_MAIN, .75, .5)
            #print(f"x:{gamestate.players[controller.port].cursor.x} y:{gamestate.players[controller.port].cursor.y} target_x:{target_x} target_y:{target_y}")
            return
        #Move left if we're too right
        if gamestate.players[controller.port].cursor.x > target_x + wiggleroom:
            controller.release_button(enums.Button.BUTTON_A)
            controller.tilt_analog(enums.Button.BUTTON_MAIN, .25, .5)
            #print(f"x:{gamestate.players[controller.port].cursor.x} y:{gamestate.players[controller.port].cursor.y} target_x:{target_x} target_y:{target_y}")
            return

        #If we get in the right area, press A
        #print(f"x:{gamestate.players[controller.port].cursor.x} y:{gamestate.players[controller.port].cursor.y} target_x:{target_x} target_y:{target_y}")
        #print(f"Stage selected: {self.stage_selected}")
    ...
    def change_controller_status(
        ...
        if ai_state.cursor_y < target_y - wiggleroom:
            controller.tilt_analog(enums.Button.BUTTON_MAIN, .5, 1)
            #print(f"x:{ai_state.cursor_x} y:{ai_state.cursor_y} target_x:{target_x} target_y:{target_y}")
            return
        #Move downn if we're too high
        if ai_state.cursor_y > target_y + wiggleroom:
            controller.tilt_analog(enums.Button.BUTTON_MAIN, .5, 0)
