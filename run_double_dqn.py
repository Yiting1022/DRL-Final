...
# 创建保存模型的目录
models_dir = os.path.join("models", "double_dqn_" + datetime.datetime.now().strftime("%Y%m%d-%H%M%S"))
...
checkpoint_callback = CheckpointCallback(
    ...
    name_prefix="double_dqn_melee_checkpoint",
...
try:
    ...
    final_model_path = os.path.join(models_dir, "double_dqn_melee_final")
    model.save(final_model_path)
    print(f"Double DQN training completed. Model saved to {final_model_path}")
except KeyboardInterrupt:
    print("Training interrupted by user")
    interrupted_model_path = os.path.join(models_dir, "double_dqn_melee_interrupted")
